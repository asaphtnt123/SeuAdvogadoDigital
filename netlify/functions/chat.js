...
exports.handler = async function(event, context) {
    ...
    try {
        ...
        const respostaLocal = generateSmartLocalResponse("erro");
        ...
        const response = await fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "Você é o Dr. Lex IA, assistente jurídico brasileiro. Seja claro, educativo e deixe claro que não substitui advogado."
                    },
                    {
                        role: "user",
                        content: message
...
function generateSmartLocalResponse(message) {
    ...
    if (containsAny(lowerMessage, ['bom dia', 'boa tarde', 'boa noite'])) {
        return `**Dr. Lex IA** ⚖️\n\n*${
            lowerMessage.includes('bom dia') ? 'Bom dia' :
            lowerMessage.includes('boa tarde') ? 'Boa tarde' :
            'Boa noite'
        }! Em que posso auxiliá-lo com questões jurídicas?*`;
    }
    ...
    // GENÉRICA
    return `**Dr. Lex IA** ⚖️\n\n🔍 **Consulta Recebida**\n\nPara **"${message}"**, recomendo:\n\n📋 **Para uma orientação precisa:**\n• Explique os fatos em ordem cronológica\n• Envie documentos relevantes\n• Informe o que você espera resolver\n\n⚠️ *Orientação educativa. Consulte advogado para análise completa.*`;
}
